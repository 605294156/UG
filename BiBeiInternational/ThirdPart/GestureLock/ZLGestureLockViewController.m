...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = kRGBColor(22, 22, 22);
    ...
    [button setBackgroundImage:UIIMAGE(@"icon_close") forState:UIControlStateNormal];
...
- (void)setupMainUI {
    ...
    if (self.unlockType==ZLUnlockTypeCreatePsw) {
        if (kWindowW<330) {
            headIcon = [[UIImageView alloc] initWithFrame:CGRectMake((self.view.frame.size.width - 70) * 0.5, SafeAreaTopHeight-20, 70, 70)];
            headIcon.layer.cornerRadius=35;
        }else{
            headIcon = [[UIImageView alloc] initWithFrame:CGRectMake((self.view.frame.size.width - 70) * 0.5, SafeAreaTopHeight, 70, 70)];
            headIcon.layer.cornerRadius=35;
        }
        ...
        if (kWindowW<330) {
            headIcon = [[UIImageView alloc] initWithFrame:CGRectMake((self.view.frame.size.width - 90) * 0.5, SafeAreaTopHeight, 90, 90)];
        }
        else{
             headIcon = [[UIImageView alloc] initWithFrame:CGRectMake((self.view.frame.size.width - 90) * 0.5, SafeAreaTopHeight+50, 90, 90)];
        }
    ...
    headIcon.image = [UIImage imageNamed:@"aboutIconImage"];
...
//    self.statusLabel = nameLabel;
    ...
    // 手势状态栏提示label
    UILabel *statusLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(gestureLockIndicator.frame), self.view.frame.size.width, 30)];
    ...
    // 九宫格 手势密码页面
    ZLGestureLockView *gestureLockView = [[ZLGestureLockView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - self.view.frame.size.width - 30 - btnH, self.view.frame.size.width, self.view.frame.size.width)];
    ...
    if ([gesturesPassword isEqualToString:[ZLGestureLockViewController gesturesPassword]]) {
        ...
        if (errorCount - 1 == 0) { // 你已经输错五次了！ 退出重新登陆！
            if (self.unlockType==ZLUnlockTypeDeletePsw) {
                errorCount = 5;
                [self.view ug_showToastWithToast:LocalizationKey(@"gesturesPassThirdPartTip9")];
...
- (void)forgetGesturesPassword:(id)sender {
    ...
    EasyShowAlertView *showView = [EasyShowAlertView showAlertWithTitle:LocalizationKey(@"tips") message:LocalizationKey(@"gesturesPassThirdPartTip13")];
        [showView addItemWithTitle:LocalizationKey(@"cancel") itemType:ShowAlertItemTypeBlodBlack callback:^(EasyShowAlertView *showview) {
