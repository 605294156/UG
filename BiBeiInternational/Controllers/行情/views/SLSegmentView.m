...
#define kCanSliderLineWidth  (kWindowW - 15 *2)/self.segementTitleArray.count
...
- (UIView *)segementContainer
{
    if (!_segementContainer) {
        ...
        _segementContainer.backgroundColor = [UIColor groupTableViewBackgroundColor];
    }
...
-(UIView *)canSliderLine
{
    if (!_canSliderLine) {
        ...
        _canSliderLine.backgroundColor = mainColor;
    }
...
-(UIView *)bottomLine
{
    if (!_bottomLine) {
        ...
        _bottomLine.backgroundColor = commonLineColor;
    }
    return _bottomLine;
}
