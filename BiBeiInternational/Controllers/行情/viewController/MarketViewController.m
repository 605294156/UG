...
- (void)viewDidLoad {
    ...
    [self setupBarButtonItemWithImageName:@"changeMoney" type:UGBarImteTypeRight callBack:^(UIBarButtonItem *item) {
    ...
    self.segmentTitleArray= @[@"USDT",@"UG",LocalizationKey(@"collect")];
    [self setUpViews];
...
-(UIScrollView *)ctrlContanier
{
    if (!_ctrlContanier) {
        _ctrlContanier = [[UIScrollView alloc] init];
         _ctrlContanier.frame=CGRectMake(0, CGRectGetMaxY(_segment.frame)+2, kWindowW, kWindowH-SafeAreaTopHeight-SafeAreaBottomHeight-40-2);
