...
@interface UGIndexView ()

@property (nonatomic, strong) UGSectionIndicatorView *indicatorView;
...
- (void)reload {
    ...
    //获取标题组
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(ug_sectionIndexTitles)]) {
        self.indexItems = [self.dataSource ug_sectionIndexTitles];
...
- (void)attributeSettings {
    ...
    //字体颜色
    if (!self.titleColor) {
        self.titleColor = [UIColor colorWithHexString:@"717171"];
    }
    //右边距
...
- (void)initialiseAllTitles {
    ...
    //设置Y坐标最小值
    self.minY = 0.0;//(CGRectGetHeight(self.frame) - totalHeight)/2.0;
    CGFloat startY = self.minY  + self.titleSpace;
    //以 'W' 字母为标准作为其他字母的标准宽高
    self.itemMaxSize = [@"W" boundingRectWithSize:CGSizeMake(UG_SCREEN_WIDTH, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont boldSystemFontOfSize:self.titleFontSize]} context:nil].size;
    //标题视图布局
...
- (void)animationView:(UIView *)view {
    ...
    if (!self.indicatorView.superview) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(ug_addIndicatorView:)]) {
            [self.delegate ug_addIndicatorView:self.indicatorView];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex {
    ...
    if (newIndex >= 0 && newIndex < self.itemsViewArray.count) {
        ...
        //回调代理方法
        if (self.isCallback && self.delegate && [self.delegate respondsToSelector:@selector(ug_selectedSectionIndexTitle:atIndex:)]) {
            [self.delegate ug_selectedSectionIndexTitle:self.indexItems[newIndex] atIndex:newIndex];
            
            if (isLetter) {
                //只有手势滑动，才会触发指示器视图
                if (!self.indicatorView) {
                    self.indicatorView = [[UGSectionIndicatorView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
                }
                self.indicatorView.alpha = 1.0f;
                [self.indicatorView ug_setOrigin:CGPointMake(UG_SCREEN_WIDTH - self.marginRight - self.titleFontSize - 10 - self.indicatorMarginRight, newItemLabel.center.y + self.frame.origin.y) title:newItemLabel.text];
...
@interface UGSectionIndicatorView()
...
@implementation UGSectionIndicatorView
