...
- (void)setMenu{
    ...
    [UIView animateWithDuration:0.6 animations:^{
        ...
        self->_closeImgView.transform = CGAffineTransformRotate(self->_closeImgView.transform, M_PI);
    }];
}
...
-(void)dismissMenu{
   ...
   self.timer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(returnUpVC) userInfo:nil repeats:YES];
    [UIView animateWithDuration:0.3 animations:^{
        //-M_PI_2*1.5
        self->_closeImgView.transform = CGAffineTransformRotate(self->_closeImgView.transform, -M_PI_2);
    }];
}
//设置按钮从后往前下落
- (void)returnUpVC{
