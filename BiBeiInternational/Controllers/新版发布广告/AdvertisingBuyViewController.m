...
@interface AdvertisingBuyViewController ()<UITableViewDelegate,UITableViewDataSource,Advertising1TableViewCellDelegate,Adversiting2TableViewCellDelegate,UITextFieldDelegate>{
...
//    [self.contentArr replaceObjectAtIndex:7 withObject:_detailModel.remark];
    ...
    if (!UG_CheckStrIsEmpty(_detailModel.remark)) {
//        [self.contentArr replaceObjectAtIndex:6 withObject:_detailModel.remark];
    }
...
//        cell.rightConstraint.constant = indexPath.row == 4 ? 0.0f : 20.0f;
...
//    }else if (indexPath.row == 0 || indexPath.row == 5 ){
        ...
        cell.leftLabel.text = [self getNameArr][indexPath.row];
        NSString *contentStr = self.contentArr[indexPath.row];
        if (indexPath.row == 0) {
            cell.centerLabel.text = contentStr;
            cell.rightLabel.text =  @"元";
        } else {
            cell.centerLabel.text = @"";
            cell.rightLabel.text =  [NSString stringWithFormat:@"%@分钟",contentStr];
        }
        return cell;
...
//        [[UIApplication sharedApplication].keyWindow  ug_showToastWithToast:@"买入总额不能小于最小限额！"];
//        return;
//    }
...
//    }
    ...
    [UGBuyOrSellPopView initWithTitle:@"我要购买" WithNumber:[NSString stringWithFormat:@"%@ UG",totalCny] WithRate:[NSString stringWithFormat:@"%@ UG / (费率 %@%@",rate,rateP,@"%)"] WithReal:[NSString stringWithFormat:@"%@ UG",[NSString ug_bySubtractFormatWithMultiplier:totalCny multiplicand:rate]] withType:YES WithHandle:^{
        ...
        advertriseApi.advertiseType = @"0";//购买0
...
//        advertriseApi.remark = self.contentArr[6];
        ...
        //        advertriseApi.advertiseType = @"BUY";//购买0
...
//                }
                ...
                if (self.index != 1) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"创建交易成功" object: nil userInfo:@{@"buyOrSell": @"0"}];
...
-(void)getUGUGJyRate{
    UGUGJyRateApi *api = [UGUGJyRateApi new];
    [api ug_startWithCompletionBlock:^(UGApiError *apiError, id object) {
        if(object){
            self.rateStr = object;
            [self upDateRate:self.contentArr[1]];
...
//             LocalizationKey(@"maximum"),
             LocalizationKey(@"buyNumber"),
             @"", //加一个 费率
             @"付款方式",
             LocalizationKey(@"payTerm"),
...
//             LocalizationKey(@"inputTradMax"),
             LocalizationKey(@"inputBuyAmount"),
             @"", //加一个 费率
             @"",
             @"付款方式",
...
- (NSMutableArray *)contentArr {
    if (!_contentArr) {
        NSString *timeStr = [self upDataMessage:@"buyMin" WithMessage:@"30"];
