...
-(void)awakeFromNib{
    ...
    self.tips.text = LocalizationKey(@"billDetailQRTip");
...
- (void)savedPhotoImage:(UIImage*)image didFinishSavingWithError: (NSError *)error contextInfo: (void *)contextInfo {
    if (error) {
        [self ug_showToastWithToast:LocalizationKey(@"savePhotoFailure")];
    }
    else {
        NSLog(@"保存图片成功");
        [self ug_showToastWithToast:LocalizationKey(@"savePhotoSuccess") ];
    }
}
