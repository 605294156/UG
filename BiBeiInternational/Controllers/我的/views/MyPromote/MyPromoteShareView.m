...
-(void)awakeFromNib{
    ...
    self.BGViewHeight.constant = (kWindowW-60)/1.36+250;
    self.saveBtnWidth.constant = kWindowW/2;
    [self.pasteButton setTitle:LocalizationKey(@"copy") forState:UIControlStateNormal];
    self.topTitleLabel.text = [NSString stringWithFormat:@"%@ %@ %@",[UGManager shareInstance].hostInfo.username,LocalizationKey(@"inviteYouJoin"),LocalizationKey(@"projectName")];
