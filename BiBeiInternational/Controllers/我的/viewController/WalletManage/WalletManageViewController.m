...
@interface WalletManageViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,WalletManageTableViewCellDelegate>{
    BOOL _refreshFlag;
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *topBgViewHeight;//最上面背景View的高度
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = LocalizationKey(@"walletManagement");
    ...
    if (!self.assetCNY) {
      ...
      self.asset2.text = [NSString stringWithFormat:@"%@",self.assetCNY];
    }
    ...
    _tableHeadView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kWindowW, 50)];
    ...
    self.totalAssets.text =  LocalizationKey(@"totalAssets");
    ...
    @weakify(self);
    [self setupBarButtonItemWithTitle:LocalizationKey(@"detail") type:UGBarImteTypeRight callBack:^(UIBarButtonItem *item) {
        @strongify(self);
...
#pragma mark - 语言切换
- (void)languageChange {
    self.navigationItem.leftBarButtonItem.title = LocalizationKey(@"detail");
    self.totalAssets.text =  LocalizationKey(@"totalAssets");
}
...
-(void)buttonIndex:(NSIndexPath *)indexPath withModel:(WalletManageModel *)model{
    ...
    if (![model.coin.canRecharge isEqualToString:@"1"] && ![model.coin.canWithdraw isEqualToString:@"1"]) {
        [self.view ug_showToastWithToast:LocalizationKey(@"notRaiseMoneyTip")];
        model.clickIndex = @"0";
    }else{
        //可以充币或提币
...
-(void)coinAlterView:(NSIndexPath*)indexPath withModel:(WalletManageModel *)model{
    ...
    if ([model.coin.canRecharge isEqualToString:@"1"] &&[model.coin.canWithdraw isEqualToString:@"1"]) {
        //可以充币，提币
        arrayTitle = @[LocalizationKey(@"chargeMoney") ,LocalizationKey(@"mentionMoney") ];
        arrayColor = @[GreenColor,RedColor];
    }else if ([model.coin.canRecharge isEqualToString:@"1"] && ![model.coin.canWithdraw isEqualToString:@"1"]){
        arrayTitle = @[LocalizationKey(@"chargeMoney")];
        arrayColor = @[GreenColor];
    }else{
        arrayTitle = @[LocalizationKey(@"mentionMoney")];
        arrayColor = @[GreenColor];
    }
    [self.view createAlertViewTitleArray:arrayTitle textColor:arrayColor font:[UIFont systemFontOfSize:16] type:1  actionBlock:^(UIButton * _Nullable button, NSInteger didRow) {
        NSLog(@"按钮标题--%@",button.currentTitle);
        ...
        }else if ([button.currentTitle isEqualToString:LocalizationKey(@"cancel")]){
            //取消
            [self cancelBtnClick];
        }
    }];
...
-(void)cancelBtnClick{
    ...
    [self.tableView reloadRowsAtIndexPaths:[NSArray arrayWithObjects:self.clickIndexPath,nil] withRowAnimation:UITableViewRowAnimationNone];
}
//MARK:--充币点击事件
-(void)chargeMoneyBtnClick{
    _refreshFlag = YES;
    ChargeMoneyViewController *chargeVC = [[ChargeMoneyViewController alloc] init];
    chargeVC.mininumChargeamount = [ToolUtil removeFloatAllZeroByString:[NSString stringWithFormat:@"%.8f",self.clickModel.coin.minRechargeAmount]];
...
- (IBAction)eyeBtnClick:(UIButton *)sender {
    ...
    if (_eyeButton.selected) {
        ...
        self.asset1.text = @"********";
        self.asset2.text = [NSString stringWithFormat:@"********"];
        ...
        if(!([[UGManager shareInstance] hasLogged])){
            //没登录不做处理
            self.asset1.text = @"0.000000";
            self.asset2.text = [NSString stringWithFormat:@"≈0.000000CNY"];
        }else{
            self.asset1.text = [NSString stringWithFormat:@"%@",self.assetUSD];
            self.asset2.text = [NSString stringWithFormat:@"%@",self.assetCNY];
        }
...
-(void)eyeButtonArrageData{
    if (!_eyeButton.selected) {
        ...
        if(!([[UGManager shareInstance] hasLogged])){
            //没登录不做处理
            self.asset1.text = @"0.000000";
            self.asset2.text = [NSString stringWithFormat:@"≈0.00CNY"];
        }else{
            self.asset1.text = [NSString stringWithFormat:@"%@",self.assetUSD];
            self.asset2.text = [NSString stringWithFormat:@"%@",self.assetCNY];
        }
