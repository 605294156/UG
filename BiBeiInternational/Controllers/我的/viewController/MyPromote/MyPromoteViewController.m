...
- (void)viewDidLoad {
    ...
    self.menuList = @[LocalizationKey(@"promoteFriends"),LocalizationKey(@"myCommission")];
    self.delegate = self;
    ...
    [self reloadScrollPage];
     self.promoteLink= [NSString stringWithFormat:@"%@/#/register?agent=%@",[UGManager shareInstance].hostInfo.promotionPrefix,[UGManager shareInstance].hostInfo.promotionCode];
    
    @weakify(self);
    [self setupBarButtonItemWithImageName:@"promoteShareImage" type:UGBarImteTypeRight callBack:^(UIBarButtonItem *item) {
        @strongify(self);
        NSLog(@"分享");
        [self shareView];
...
-(void)pasteBtnClick{
    ...
    [_shareView ug_showToastWithToast:LocalizationKey(@"pastePromoteLink")];
}
//MARK:--字符串生成二维码
...
//    if (error) {
        [_shareView ug_showToastWithToast:LocalizationKey(error ?  @"savePhotoFailure" : @"savePhotoSuccess")];
//    }
//    else {
//        NSLog(@"保存图片成功");
//        [_shareView ug_showToastWithToast:LocalizationKey(@"savePhotoSuccess")];
//    }
}
