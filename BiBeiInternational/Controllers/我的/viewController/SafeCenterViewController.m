...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = LocalizationKey(@"safeCenter");
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    NSString*Gesturepassword=[ZLGestureLockViewController gesturesPassword].length >0 ? LocalizationKey(@"opened") : LocalizationKey(@"unOpened");
...
-(NSArray *)getNameArr{
    NSArray * nameArr = @[ LocalizationKey(@"gesturesPassword")];
    return nameArr;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell.rightLabel.text isEqualToString:LocalizationKey(@"unOpened")]) {
