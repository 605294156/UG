...
//static  NSString * s_host = @"api.test.bibei.ph";//聊天
static  NSString * s_host = @"api.ugcoin.pro";//聊天
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err{
    ...
    //[APPLICATION.window ug_showToastWithToast:@"聊天类socket断开连接"];
...
- (void)didReadData:(NSData *)data withTag:(long)tag{
    ...
    while (_readBuf.length >= 22)//因为头部固定22个字节，数据长度至少要大于22个字节，我们才能得到完整的消息描述信息
    {
        ...
        DLog(@"聊天完整长度 complateDataLength&%ld-----_readBuf&%lu",(long)complateDataLength,(unsigned long)_readBuf.length);
...
-(void)handleTcpResponseData:(NSData*)data withTag:(long)tag{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(ChatdelegateSocket:didReadData:withTag:)]) {
        [_delegate ChatdelegateSocket:gcdSocket didReadData:data withTag:tag];
    }
